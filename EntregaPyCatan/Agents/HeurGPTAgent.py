...
class HeurGPTAgent(AgentInterface):
    ...
    GAME_START_CANDIDATE_LIMIT = 10
    MOVE_THIEF_CANDIDATE_LIMIT = 6
    ...
    def _game_start_state(self, board_instance, valid_nodes, valid_roads, existing_pips, existing_numbers):
        candidate_nodes = []
            ...
            candidate_nodes.append(
                {
                    "node_id": node_id,
                    "harbor": board_instance.nodes[node_id]["harbor"],
                    "road_options": valid_roads[node_id],
                    "resource_pips": resource_pips,
                    "numbers": sorted(numbers),
                    "terrains": [
                        {
                            "type": board_instance.terrain[terrain_id]["terrain_type"],
                            "prob": board_instance.terrain[terrain_id]["probability"],
                        }
                        for terrain_id in board_instance.nodes[node_id]["contacting_terrain"]
                    ],
                    "heuristic_score": round(
                        self._node_score_for_settlement(
                            node_id,
                            board_instance=board_instance,
                            existing_resource_pips=existing_pips,
                            existing_numbers=existing_numbers,
        ...
        candidate_nodes.sort(key=lambda item: item["heuristic_score"], reverse=True)
        return {
            "player_id": self.id,
            "candidate_nodes": candidate_nodes[: self.GAME_START_CANDIDATE_LIMIT],
            "existing_pips": existing_pips,
            "existing_numbers": sorted(existing_numbers),
        }

    def _move_thief_state(self):
        thief_terrain = next((terrain["id"] for terrain in self.board.terrain if terrain["has_thief"]), -1)
        my_nodes = [node["id"] for node in self.board.nodes if node["player"] == self.id]
        my_cities = [node["id"] for node in self.board.nodes if node["player"] == self.id and node["has_city"]]
        my_towns = [node_id for node_id in my_nodes if node_id not in my_cities]
        return {
    ...
    def _request_llm(self, prompt, context="llm_request"):
        if self.client is None:
            if self.llm_debug:
                print(
                    f"[{self.__class__.__name__} P{self.id}] {context}: client unavailable, using heuristic fallback"
                )
            return None
    ...
    def on_moving_thief(self):
        ...
        heuristic_context = {
            ...
            "top_candidates": heuristic_candidates[: self.MOVE_THIEF_CANDIDATE_LIMIT],
        }
        ...
        prompt = self._prompt_by_size("MOVE_THIEF_PROMPT").format(
            board_state=self._json_dump(self._move_thief_state()),
        ...
        response_content = self._request_llm(prompt, context="move_thief")
    ...
    def on_game_start(self, board_instance):
        ...
        prompt = self._prompt_by_size("GAME_START_PROMPT").format(
            player_id=self.id,
            board_state=self._json_dump(
                self._game_start_state(
                    board_instance,
                    valid_nodes,
                    valid_roads,
                    existing_pips,
        ...
        response_content = self._request_llm(prompt, context="game_start")
